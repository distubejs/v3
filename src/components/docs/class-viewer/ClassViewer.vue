<template>
  <div v-if="clarse" id="class-viewer" class="docs-page">
    ...
    <div id="class-constructor" v-if="clarse.construct && (showPrivate || clarse.construct.access !== 'private')">
      <h2>Constructor</h2>
      <pre><code class="js">new {{ docs.global && docs.global !== clarse.name ? docs.global + "." : "" }}{{ clarse.name }}{{ tag.includes("alpha") && clarse.name === docs.global ? ".default" : "" }}(<span class="constructor-param" v-for="param in constructorParams" :key="param.name">{{ param.name }}</span>);</code></pre>
